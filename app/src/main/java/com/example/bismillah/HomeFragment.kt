...
class HomeFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        askPermission(android.Manifest.permission.READ_EXTERNAL_STORAGE) {
            ...
            root.simple_recycle.adapter = adapter
    ...
    override fun onResume() {
        super.onResume()
        if (ContextCompat.checkSelfPermission(root.context, android.Manifest.permission.READ_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
        ...
        root.lovely_recycle.adapter = RvLoveAdapter(root.context,list,object :RvItemClickLove{
            override fun itemClick(music: Music, position: Int) {
                findNavController().navigate(R.id.playerFragment,
                    bundleOf("music" to music,"position" to position))
            }
    ...
    @SuppressLint("Range", "Recycle", "InflateParams")
    fun getAudioFiles() {
        ...
        root.simple_recycle.adapter = RvAdapter(root.context,audioArrayList,object : RvItemClick{
            override fun itemClick(music: Music, position: Int) {
                Toast.makeText(context, "Clicked", Toast.LENGTH_SHORT).show()
            }
        })
        root.lovely_recycle.adapter = RvLoveAdapter(root.context,loveListData,object : RvItemClickLove{
            override fun itemClick(music: Music, position: Int) {
                findNavController().navigate(R.id.playerFragment,
                bundleOf("music" to  music, "position" to position))
            }
...
object MyData {
    ...
    var loveListData:ArrayList<Music> = ArrayList()
}